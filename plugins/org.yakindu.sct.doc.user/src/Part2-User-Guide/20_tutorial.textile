...
h2(#oss_introducing-advanced-concepts). Introducing advanced concepts

This section introduces some advanced concepts of the "statechart language":30_statechart_language.html by enhancing the light switch example.

It has been inspired by https://www.gira.de/gebaeudetechnik/produkte/automatische-lichtsteuerung/bewegungsmelder-360-ap.html.


h3(#oss_introducing-variables-guard-conditions-and-actions). Introducing variables, guard conditions, and actions

Add dimmer functionality to the light switch.

The light switch receives another switch so that it now consists of two switches. The _on_ switch turns the light on and, when pressed repeatedly, increases the brightness in steps up to a maximum. The _off_ switch turns the light off.

This section introduces variables (to store the brightness), guard conditions (to increase the brightness or not, depending on the value of the brightness variable), and modifying the brightness variable).

h3(#oss_introducing-interfaces-timing-expressions-and-composite-states). Introducing interfaces, timing expressions, and composite states

Add a motion sensor.

If the light is turned off and the motion sensor triggers, the light is turned on with its lowest brighness. 30 seconds after the last detected motion the light is turned off. Operating the switch manually overrides the motion sensor mode, i.&nbsp;e. if the light is switched on manually or the dimmer functionality is used, it won't be switched of automatically. Turning the light off manually enganges the motion sensor again.

This section introduces interfaces – one for the user, another one for the motion sensor –, timing expressions to automatically turn off the light, and composite states dealing with the ordinary light switch functionality in one composite state and dealing with the motion sensing functionality in the other composite state.

h3(#oss_introducing-history-states). Introducing history states

Add a twilight switch.

During darkness (= sensor signals brightness values below a certain threshold for a certain time), the light is automatically turned on. It is turned off again in the morning after brightness has crossed the threshold for some time. It is not sensible to operate the light switch in motion sensing mode and in twilight mode at the same time.

This section serves as a prerequisite for the next one.

h3(#oss_introducing-operations). Introducing operations

Add presense simulation.

During darkness the light is turned on and off at random over "reasonable" periods of time, but only during the evening and morning hours. At night the light is turned on for a few minutes only, if at all. Manually turning on the light switch overrides the presence simulation. When turning off the light, the presence simulation is engaged again.

This chapter introduces operations which are used to access data not available in the statechart language, i.&nbsp;e. time of day and random numbers.

h3(#oss_uncovered). Uncovered concepts

Orthogonal states, choice, synchronization, raising an event, built-in functions and more are not covered by enhancing the light switch example yet. Ideas are welcome!
