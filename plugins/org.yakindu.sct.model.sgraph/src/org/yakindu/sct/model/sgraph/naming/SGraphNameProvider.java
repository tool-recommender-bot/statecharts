...
public class SGraphNameProvider extends DefaultDeclarativeQualifiedNameProvider {
    ...
    public QualifiedName qualifiedName(final Transition ele) {

        final int index = ((State) ele.eContainer()).getOutgoingTransitions().indexOf(ele);
        String name = _TRANSITION_NAME + "to_" + ele.getTarget().getName() + "_" + index;

        final QualifiedName qualifiedNameFromConverter = QualifiedName.create(name);
