...
/**
 * Introduced to apply rename refactorings, triggered from an Xtext language, to
 * SCT specifications. In difference to {@link DefaultReferenceUpdater} this
 * implementation does not check if this is a XtextResource. The references
 * processed during execution have to be instantiated by Xtext before.
 */
...
public class SCTXtextSpecificationReferenceUpdater extends DefaultReferenceUpdater {
	...
	protected void createReferenceUpdates(final ElementRenameArguments elementRenameArguments,
		...
		final SubMonitor progress = SubMonitor.convert(monitor, "Creating SCT specification updates",
	...
	private void acceptChangesPerResource(final ResourceSet resourceSet,
		...
		for (final URI changedResource : changesPerResource.keySet()) {
			...
			final CompositeChange compositeChange = createResourceChange(resourceSet, changedResource,
					changesPerResource.get(changedResource));
	...
	protected CompositeChange createResourceChange(final ResourceSet resourceSet, final URI changedResource,
		...
		// sorts by offsets descending to ensure multiple changes can be applied to the
		// same specification feature
	...
	protected void createTextChange(final ITextRegion referenceTextRegion, final String newReferenceText,
		...
		final SpecificationElement specificationElement = EcoreUtil2.getContainerOfType(referringElement,
				SpecificationElement.class);
		if (newReferenceText != null) {
			SCTSpecificationChange specificationChange = new SCTSpecificationChange(referenceTextRegion,
					specificationElement, newReferenceText);
